...
1). a value $$z^{(i)}$$ is generated from some prior distribution $$ p_{\theta^*}(z) $$ <br>
...
So, ALL WE NEED is compute PDF of $$ p_\theta (z \mid x) = \frac{p_\theta (x \mid z) p_\theta (z)}{ p_\theta (x)} $$. But it is impossible to compute when treated in neural network.

- Why $$ p_\theta(z \mid x) $$ is impossible to compute?
...
For detouring, paper introduce a recognition model $$q_\phi (z \mid x)$$: an approximation to the intractable true posterior $$p_\theta(z \mid x)$$. In this paper, we will therefor also refer to the recognition model $$q_\phi (z \mid x)$$ as a probabilistic encoder, since given a datapoint x, it produces a distribution over the possible values of the $$z$$ from datapoint $$x$$. And we call $$p_\theta(x \mid z)$$ as a probabilistic decoder, since given $$z$$, it produces a distribution over the possible corresponding value of $$x$$. About what $$q_\phi (z \mid x)$$ looks like is treated in 2.4

### Do some math! (ELBO and KL-divergnece) - paper 2.2

To get $$q_\phi (z \mid x)$$ and approximate $$p_\theta(z \mid x)$$, we need variational inference to know how our approximation works well.
...
$$
\log p_θ(x^{(i)}) = D_{KL}(q_\phi(z \mid x^{(i)}) \ || \ p_θ(z \mid x^{(i)})) + \mathcal{L}(θ, \phi; x^{(i)})
$$

- $$\log p_θ(x^{(i)})$$: Our target log-likelihood
- $$D_{KL}(q_\phi(z \mid x^{(i)}) \ || \ p_θ(z|x^{(i)}))$$: KL divergence between real distribution and detour one.
...
$$
\log p_θ(x^{(i)}) ≥ \mathcal{L}(θ, \phi; x^{(i)}) = \mathbb E_{q_\phi(z \mid x)}
...
$$
\mathcal L(θ, \phi; x^{(i)}) = −D_{KL}(\ q_\phi(z \mid x^{(i)})\ \mid \mid \ p_θ(z)\ ) + \mathbb E_{q_\phi(z \mid x^{(i)})}
[\log p_θ(x^{(i)} \mid z)]
$$

- $$−D_{KL}(q_\phi(z \mid x^{(i)}) \ \mid \mid \ p_θ(z))$$: KL divergence between encoder distribution $$q_\phi(z \mid x^{(i)})$$ and original distribution $$p_θ(z)$$
- $$E_{q_\phi(z \mid x^{(i)})}[\log p_θ(x^{(i)} \mid z)]$$: reconstruction likelihood. Extent of decoder reconstructing $$x$$ from $$z$$.
...
We want to maximize ELBO($$E_{q_\phi(z \mid x)}[\log p_θ(x \mid z)]$$). But it’s sampling process it stochastic, it is impossible to get derivative.

For this reason, we did not sample from direct distribution, we sample from deterministic. 

Let $$z$$ be a continuous random variable and $$z ∼ q_\phi(z \mid x)$$ be some conditional distribution, and it is possible to express the random variable $$z$$ as a deterministic variable.
...
For example, let $$q_\phi(z \mid x)=N(μ_\phi(x),σ_\phi(x)^2)$$. But it is impossible to do sampling from $$z∼q_\phi(z \mid x)$$. We can change it as deterministic form like below.
...
$$
E_{z∼q_ϕ(z \mid x)}[\log p_θ(x \mid z)]=E_{ε∼N(0,1)}[\log p_θ(x \mid g_ϕ(ε,x))]\ ,\ \ \ \ \ \ g_ϕ(ε,x) = μ_ϕ(x)+σ_ϕ(x)⋅\epsilon, \ \ \ \ \epsilon∼N(0,1)
$$
...
We estimate approximate posterior as $$q_\phi(z \mid x)$$ and we can reparameterize $$\phi$$ as a deterministic. Now we want to estimate sampling function(decoder).

We can now form Monte Carlo estimates of expectation of some function $$f(z)$$ that $$q_\phi(z \mid x)$$.
...
→ 여전히 고차원 벡터 형태이다. 따라서 인코더 $$q_ϕ(z \mid x)$$가 추정하는 분포는 대부분 다변량 정규 분포이다.
...
This is non-linear and hard to compute direct. For this reason, in variational inference, rewrite the equation by adding $$q_\phi(z \mid x)$$. 
...
$$
=\log\mathbb E_{q_ϕ(z \mid x)}\Biggr[ \frac{p_θ(x,z)}{q_ϕ(z \mid x)}\Biggr] = \mathbb E_{q_ϕ(z \mid x)}[\log p_θ(x,z)− \log q_ϕ(z \mid x)]
$$
...
$$
\log p_θ(x) ≥ \mathbb E_{q_ϕ(z \mid x)}[\log p_θ(x,z)− \log q_ϕ(z \mid x)]
$$
