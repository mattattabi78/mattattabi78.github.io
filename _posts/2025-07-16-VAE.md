...
$$
\mathcal L(θ, \phi; x^{(i)}) = −D_{KL}(\ q_\phi(z|x^{(i)})\ || \ p_θ(z)\ ) + \mathbb E_{q_\phi(z \mid x^{(i)})}
[\log p_θ(x^{(i)} \mid z)]
$$

- $$−D_{KL}(q_\phi(z \mid x^{(i)}) \ || \ p_θ(z))$$: KL divergence between encoder distribution $$q_\phi(z \mid x^{(i)})$$ and original distribution $$p_θ(z)$$
